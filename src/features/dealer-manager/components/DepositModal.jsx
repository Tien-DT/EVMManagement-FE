...
const DepositModal = ({ visible, order, onClose, onSuccess }) => {
  ...
  const remainingAmount = order.totalAmount
    ? order.totalAmount - depositAmount
    : 0;
  ...
  const handleDeposit = async () => {
    ...
    if (depositAmount < SEPAY_MIN_AMOUNT) {
      const errorMsg = `Số tiền đặt cọc tối thiểu là ${SEPAY_MIN_AMOUNT.toLocaleString(
        "vi-VN"
      )} ₫. Số tiền đặt cọc hiện tại: ${depositAmount.toLocaleString(
        "vi-VN"
      )} ₫`;
    ...
    try {
      ...
      if (transactionCode && paymentUrl) {
        ...
        message.error(
          "Không tìm thấy link thanh toán trong response. Vui lòng kiểm tra console."
        );
      }
    } catch (error) {
      ...
      <Spin spinning={loading}>
        ...
        <Descriptions bordered column={1} size="small">
          ...
          <Descriptions.Item label="Số tiền đặt cọc (10%)">
            <span
              style={{ fontSize: "18px", fontWeight: 700, color: "#fa8c16" }}
            >
        ...
        {depositAmount < SEPAY_MIN_AMOUNT && (
          <Alert
            ...
            description={`Số tiền đặt cọc (${depositAmount.toLocaleString(
              "vi-VN"
            )} ₫) nhỏ hơn mức tối thiểu của SEPay (${SEPAY_MIN_AMOUNT.toLocaleString(
              "vi-VN"
            )} ₫). Vui lòng kiểm tra lại tổng tiền đơn hàng trong cơ sở dữ liệu.`}
            type="error"
            showIcon
            style={{ marginTop: 16 }}
          />
        ...
        <Alert
          ...
          description={
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              ...
              <li style={{ color: "#ff4d4f", fontWeight: 500 }}>
                Lưu ý: Số tiền đặt cọc tối thiểu là{" "}
                {SEPAY_MIN_AMOUNT.toLocaleString("vi-VN")} ₫
              </li>
            </ul>
          }
          type="warning"
