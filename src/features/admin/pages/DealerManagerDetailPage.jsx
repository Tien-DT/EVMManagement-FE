    ...
    const fetchManagerDetails = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          endpoints.userProfile.getById(id)
        );
        ...
        const managerWithEmail = {
          ...
          email:
            managerData.email ||
            managerData.account?.email ||
            managerData.accountEmail ||
            null,
        };
        ...
        if (managerData.dealerId) {
          try {
            const dealerResponse = await axiosInstance.get(
              endpoints.admin.dealers
            );
            const dealers =
              dealerResponse?.data?.items || dealerResponse?.data || [];
        ...
        if (idError?.response?.status === 404 && accountId) {
          try {
            await axiosInstance.delete(
              endpoints.userProfile.getById(accountId),
              {
                params: { isDeleted: true },
              }
            );
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {manager.fullName || manager.name || "No name"}
                </h1>
                <div className="mt-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                <button className="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-md font-medium hover:bg-red-700 transition">
            ...
            {manager.dealer && (
              <div className="flex items-start md:col-span-2">
                ...
                <div>
                  ...
                  <div className="text-gray-900 mt-1 font-medium">
                    {manager.dealer.name || "No dealer"}
                  </div>
                  {manager.dealer.code && (
                    <div className="text-xs text-gray-500 mt-1">
                      Mã: {manager.dealer.code}
                    </div>
                  )}
                </div>
              </div>
