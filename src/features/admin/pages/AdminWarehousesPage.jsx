...
const AdminWarehousesPage = () => {
  ...
  // Filter chỉ hiển thị EVM warehouses
  const evmWarehouses = warehouses.filter(w => w.type === 'EVM');
  
  const filteredWarehouses = evmWarehouses.filter(
    (warehouse) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Quản lý Kho EVM</h1>
          <p className="text-gray-600 mt-1">Quản lý các kho hàng EVM trong hệ thống</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-md border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {evmWarehouses.reduce((sum, w) => sum + (w.currentStock || w.vehicles?.length || 0), 0)}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-md border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {evmWarehouses.reduce((sum, w) => sum + (w.capacity || 0), 0)}
          </p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredWarehouses.map((warehouse) => {
                const capacityPercentage = getCapacityPercentage(
                  warehouse.currentStock || warehouse.vehicles?.length || 0,
        ...
        {pagination.totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Hiển thị {filteredWarehouses.length} trong tổng số {evmWarehouses.length} kho EVM
            </div>
            <div className="flex items-center space-x-2">
              <button
