...
export const warehouseService = {
  ...
  // Add vehicles to warehouse (EVM Staff specific)
  addVehicles: async (warehouseId, vehicles) => {
    try {
      const response = await axiosInstance.post(
        `/v1/Warehouses/evm/add-vehicles`,
        {
          warehouseId,
          vehicles,
        }
      );
      console.log("Add vehicles to warehouse response:", response);
      return response;
    } catch (error) {
      console.error("Add vehicles to warehouse error:", error);
      throw error;
    }
  },
};
