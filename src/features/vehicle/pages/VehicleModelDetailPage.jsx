...
export default function VehicleModelDetailPage() {
  ...
  const title = useMemo(() => (model ? `${model.name} ${model.code ? `(${model.code})` : ""}` : "Model Xe"), [model]);

  if (loading) return <div>Đang tải...</div>;
  if (error) return <div className="text-red-600">{String(error.message || error)}</div>;
  if (!model) return <div>Không tìm thấy</div>;
    ...
    <div className="space-y-8 bg-gray-50 min-h-screen p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Chi tiết model xe và các phiên bản</p>
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-6">
                <div>
                  <dt className="text-sm font-medium text-gray-500 uppercase tracking-wide">Mã</dt>
                  <dd className="mt-1 text-lg font-semibold text-gray-900">{model.code || "-"}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500 uppercase tracking-wide">Ngày ra mắt</dt>
                  <dd className="mt-1 text-lg font-semibold text-gray-900">{model.launchDate ? new Date(model.launchDate).toLocaleDateString() : "-"}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500 uppercase tracking-wide">Xếp hạng</dt>
                  <dd className="mt-1">
              ...
              <div className="space-y-6">
                <div>
                  <dt className="text-sm font-medium text-gray-500 uppercase tracking-wide">Trạng thái</dt>
                  <dd className="mt-1">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${model.status ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                      {model.status ? "Hoạt động" : "Không hoạt động"}
                    </span>
                  </dd>
                </div>
                <div className="md:col-span-2">
                  <dt className="text-sm font-medium text-gray-500 uppercase tracking-wide">Mô tả</dt>
                  <dd className="mt-1 text-lg text-gray-900">{model.description || "Chưa có mô tả"}</dd>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">Các phiên bản</h2>
          <span className="text-sm text-gray-500">{variants.length} phiên bản</span>
        </div>
        {variants.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Chưa có phiên bản</h3>
            <p className="text-gray-500 mb-6">Thêm các phiên bản màu và thông số kỹ thuật cho model xe này</p>
            <Link
              to={`/admin/vehiclemodels/${id}/variants/new`}
